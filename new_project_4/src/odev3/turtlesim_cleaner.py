...
def go_to_goal(velocity_publisher, x_goal, y_goal):
    ...
    while (True):
        ...
        #linear_speed = K_linear olursa hedefe gidene kadar sabit hızlı
        #distance * K_linear ise hedefe uzakken hızlı yaklaştıkça yavaşlıyor.
...
def gridClean(publisher):
    ...
    # Başlangıç pozisyonu: 1.1 noktasına git
    go_to_goal(publisher, 1, 1)

    # Yukarıya doğru 1,9 noktasına git
    setDesiredOrientation(publisher, 10, 90)  # 90 derece yukarı bak
    move(publisher, 1.0, 9.0, True)  # Yukarıya doğru 8 birim hareket (1,9'a gitmek)
    ...
    rospy.loginfo("Grid temizleme tamamlandı.")
...
if __name__ == '__main__':
    try:
        ...
        #move(velocity_publisher, 1.0, 4.0, True)#kaç birim gittiği
        #rotate(velocity_publisher, 30, 90, False)#saat yönü veya tersi dönme yapıyor
        #go_to_goal(velocity_publisher, 2.0, 1.5)#hedefe varma
        #setDesiredOrientation(velocity_publisher, 30, 90)#istenilen açıya robotu getirme
        #spiralClean(velocity_publisher, 4, 0)#spiral dönme işlemi
